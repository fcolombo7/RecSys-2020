...
def runParameterSearch_Collaborative(recommender_class, URM_train, ICM_train=None, URM_train_last_test = None, metric_to_optimize = "PRECISION",
    ...
    try:
       ...
       ##########################################################################################################
        ...
        if recommender_class in [LinearHybrid001, LinearHybrid002, LinearHybrid002ggg]:
            hyperparameters_range_dictionary = {}
        ...
        if recommender_class is HybridCombinationSearch:
            hyperparameters_range_dictionary = {}
            hyperparameters_range_dictionary["alpha"] = Real(low=0, high=1, prior='uniform')
            hyperparameters_range_dictionary["l1_ratio"] = Real(low=0, high=1, prior='uniform')

            recommender_input_args = SearchInputRecommenderArgs(
                CONSTRUCTOR_POSITIONAL_ARGS=[URM_train, ICM_train, list_rec],
                CONSTRUCTOR_KEYWORD_ARGS={},
                FIT_POSITIONAL_ARGS=[],
                FIT_KEYWORD_ARGS={}
            )
        ...
        if recommender_class is SLIM_BPR_Cython:
            ...
            hyperparameters_range_dictionary = {}
            hyperparameters_range_dictionary["topK"] = Integer(5, 1000)
            hyperparameters_range_dictionary["epochs"] = Categorical([500])#Categorical([1500])
            hyperparameters_range_dictionary["symmetric"] = Categorical([True, False])
            hyperparameters_range_dictionary["sgd_mode"] = Categorical(["adagrad", "adam"])#Categorical(["sgd", "adagrad", "adam"])
            ...
            hyperparameters_range_dictionary["learning_rate"] = Categorical([0.01, 0.001, 0.0001])#Real(low = 1e-4, high = 1e-1, prior = 'log-uniform')
